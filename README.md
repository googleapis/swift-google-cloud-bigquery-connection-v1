...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-bigquery-connection-v1.git --from 0.1.0-preview
```

Then add `GoogleBigQueryConnectionV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleBigQueryConnectionV1 <target-name> --package swift-google-cloud-bigquery-connection-v1
```
